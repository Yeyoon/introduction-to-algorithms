...
class PriorityQueue:
    ...
    def pop(self):
        ...
        left = 2 * index
        right = left + 1
        while left <= len(self.queue):
            if right > len(self.queue):
                self.queue[index-1],self.queue[left-1] = self.queue[left-1],self.queue[index-1]
                index = left
                break
            elif right <= len(self.queue):
                if self.queue[left-1] > self.queue[right-1]:
                    self.queue[right - 1],self.queue[index-1] = self.queue[index-1],self.queue[right-1]
                    index = right
                else:
                    self.queue[left-1],self.queue[index-1] = self.queue[index-1],self.queue[left-1]
                    index = left
                
                left = 2 * index
                right = left + 1
        
        popped_key = self.queue.pop(index-1)
        ...
        #self._max_heapify(0,self.queue,False)
        #self.min_index = 0
        #return popped_key
    ...
    def _find_min(self):
        ...
        #node_size = len(self.queue)
        #start = node_size // 2
        #end = 0
        #i = 0
        #for i in xrange(start,-1,-1):
        #    self._max_heapify(i,self.queue,False)
        
        self.min_index = 0

    def _max_heapify(self,index, node, isMax = True):
        left = index + index
        right = left + 1

        largest = None
        if left < len(node) and ((isMax and node[left] > node[index]) or (isMax == False and node[left] < node[index])):
            largest = left
        else:
            largest = index

        if right < len(node) and ((isMax and node[right] > node[largest]) or (isMax == False and node[right] < node[largest])):
            largest = right

        if largest != index:
            node[index],node[largest] = node[largest],node[index]
            self._max_heapify(largest,node, isMax)
